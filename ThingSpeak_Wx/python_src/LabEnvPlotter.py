...
for numdays in dayperiod:
    ...
    anom = dfsub['TempB']-dfsub['TempB'].mean()
    ...
    anom = (dfsub['BPB']-dfsub['BPB'].mean())/100
...
for numdays in dayperiod:
    ...
    ax1.fill_between(dfsub.index.values,dfsub['RH_AQ']-station_accuracy[stations[2]]['RH'],
                     dfsub['RH_AQ']+station_accuracy[stations[2]]['RH'],
                     color='grey',alpha=.25)
    ax1.set_ylabel('RH (%)')
    ax2.plot(dfsub.index.values,dfsub['Temp_AQ'],'r')
    ax2.plot(dfsub.index.values,dfsub['TempB'],'r',alpha=.9)
    ax2.fill_between(dfsub.index.values,dfsub['Temp_AQ']-station_accuracy[stations[2]]['Temp'],
                     dfsub['Temp_AQ']+station_accuracy[stations[2]]['Temp'],
                     color='grey',alpha=.25)
    ax2.set_ylabel('Temperature (degC)')
    ax3.plot(dfsub.index.values,dfsub['BP_AQ'],'k')
    ax3.plot(dfsub.index.values,dfsub['BPB'],'k',alpha=.9)
    ax3.fill_between(dfsub.index.values,dfsub['BP_AQ']-station_accuracy[stations[2]]['Baro'],
